...
**Per-session isolation of `auto_send_mode`.** The hook fallback (`per-tag pin → global → 'pause'`) is the protocol the hook script uses, but in steady state the global fallback is almost never hit — three guarantees in the server + monitor keep per-tag pins authoritative:
