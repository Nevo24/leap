...
| CLI context state | `.storage/sockets/<tag>.context` (JSON `{used_tokens, window, model}`; written on each status-line render by `leap-claude-statusline.py` (Claude) or `leap-copilot-statusline.py` (Copilot), read by the monitor's Context column. For Claude this is the authoritative source for the resolved context window (1M vs 200K - the only place Claude exposes it), except that a recorded window smaller than the recorded usage is impossible (the API rejects over-window prompts; Claude's payload sometimes misreports 200K for a session really on 1M) and is healed to 1M at read time by `claude_statusline_context_usage`; Claude's provider falls back to transcript parsing when the file is absent (not yet reconfigured, older Claude build). Codex/Gemini read context usage from their transcripts; Cursor shows N/A) |
