...
| CLI context state | `.storage/sockets/<tag>.context` (JSON `{used_tokens, window, model}`; written on each status-line render by `leap-claude-statusline.py` (Claude), `leap-copilot-statusline.py` (Copilot), or `leap-cursor-statusline.py` (cursor-agent - registered as a Claude-compatible `statusLine` command in `~/.cursor/cli-config.json`; the encrypted session store offers no transcript fallback), read by the monitor's Context column. For Claude this is the authoritative source for the resolved context window (1M vs 200K - the only place Claude exposes it), except that a recorded window smaller than the recorded usage is impossible (the API rejects over-window prompts; Claude's payload sometimes misreports 200K for a session really on 1M) and is healed to 1M at read time by `claude_statusline_context_usage`; Claude's provider falls back to transcript parsing when the file is absent (not yet reconfigured, older Claude build). Codex/Gemini read context usage from their transcripts. VS Code Copilot editor-GUI rows get context from the chat op-log instead (see the `cursor-editor-agent-tabs` skill); Cursor editor-GUI rows show N/A - no on-disk signal) |
| Model price cache | `.storage/model_prices.json` (trim of LiteLLM's `model_prices_and_context_window.json` to the model ids the CLIs report - `claude-*` / `gpt-*` / `o*` / `gemini-*`; refreshed in a background daemon thread by `utils/pricing.py` when older than 1 day, re-checked periodically so a long-running monitor refreshes without a restart, and overlays the vendored `assets/model_prices.json` snapshot. Drives the monitor Context-cell cost estimate for Claude, Codex, and Gemini sessions. Prices are NOT hardcoded - a failed/offline fetch falls back to the vendored snapshot) |
...
| Headroom-enabled marker | `.storage/headroom_enabled` (zero-byte sentinel; present iff ≥1 CLI is routed through the Headroom proxy. Written/removed by `leap --headroom`. `configure-shell-helper.sh` gates the managed-block proxy autostart on it, so the autostart is regenerated on every install/update/reconfigure and survives block rewrites. See **Headroom**) |
...
- **Adding / removing / reordering a session-table column** → All column layout is derived from one ordered table, `COLUMNS`, in `monitor/columns.py` (the single source of truth). Edit only that table - add/remove/reorder a `ColumnSpec` (its `index`, `label`, `mono`, `toggleable`, `group`) - and every consumer follows automatically: `MonitorWindow` re-exports the `COL_*` indices and `_HEADER_LABELS` / `_NON_TOGGLEABLE_COLS` from it; `_mixins/table_builder_mixin.py` sources `_CENTER_COLS` / `_MONO_COLS`; `ui/table_helpers.py` sources `COLUMN_GROUPS` (separators). Import-time asserts in `columns.py` catch a renumbered index or a non-contiguous group, and `tests/unit/test_columns.py` pins the current layout - update its golden values in the same change. Indices stay plain `int` (not `IntEnum`) so behavior is byte-identical to the old inline constants; the long-standing `self.COL_*` access is unchanged.
...
- Installs headroom via `pipx` (pinned to Python 3.13 - its native deps don't build on 3.14 yet).
...
- **Proxy autostart + health watchdog are part of the managed shell block**, not a separate island. Two repo scripts do the work: `leap-headroom-up.sh` (start-or-recycle the proxy on port 8787) and `leap-headroom-watchdog.sh` (a single-instance background loop, atomic-mkdir-locked, that re-runs the check every 5 min so a mid-session wedge self-heals). "Unhealthy" = not answering on its port, OR its log shows headroom's HTTP/2 stream-exhaustion wedge (`Max outbound streams is 100` - a state where the port still listens and a bare `GET /` still returns 421 but every upstream request fails), OR up >24h (backstop). Because the scripts run from the repo via `$LEAP_PROJECT_DIR`, `leap --update` (a `git pull`) refreshes their logic automatically; `.update-after-pull` + `reconfigure` call `make .restart-headroom` so the running watcher picks up the new code.
- **The `.storage/headroom_enabled` marker is the single switch.** It exists iff ≥1 CLI is routed (port 8787 in `cli_env.json`). `configure-shell-helper.sh` emits the two autostart lines *inside* the regenerated managed block only when the marker is present - so the autostart self-heals, survives every block rewrite, and disappears the moment routing is fully toggled off. `leap --headroom` step 3 writes/removes the marker, re-runs `configure-shell-helper.sh --update` to refresh the block immediately, and starts (or stops) the proxy+watcher to match. Runtime state (locks, `started_at`, `proxy.log`) lives under `~/.headroom/`; only the scripts moved to the repo.
- Teardown is `uninstall-helper.sh` `remove_headroom()` (called by `make uninstall`): the managed-block autostart goes with `remove_shell_config` (it also strips the legacy `# >>> leap-headroom >>>` block for pre-migration users), then it stops the proxy+watcher, removes the marker, and clears Headroom keys from `cli_env.json`. Wiring lives in `leap-select.sh` + `leap-main.sh` (dispatch + `--help`) and `_leap` (completion).
...
Optional, on by default (`show_editor_gui_agents`; legacy `show_cursor_gui_agents` migrated at startup): the monitor shows one read-only row per open Cursor *editor* Agent/Composer tab AND per recently-active VS Code Copilot Chat session - live status, PR tracking, one-click jump to the exact chat, and (VS Code only) editor-side rename via the tag context menu. Pure display overlay (no PTY, server, or queueing); rows carry `row_type == 'cursor_agent_gui'` / `'vscode_copilot_gui'`. VS Code "close" is an honest Leap-side row removal (UUID-keyed `vscode_gui_hidden` pref, auto-returns on new activity) - VS Code exposes no reliable archive/close-by-id to an extension; the scan also drops chats the user archived in VS Code itself (read from `agentSessions.state.cache`). Full design - the on-disk scans, status mappings, jump/rename via the shared Leap extension, synthetic-row reconciliation, and the two close buttons - is in the `cursor-editor-agent-tabs` skill.
