...
Optional, on by default (`show_editor_gui_agents`; legacy `show_cursor_gui_agents` migrated at startup): the monitor shows one read-only row per open Cursor *editor* Agent/Composer tab AND per recently-active VS Code Copilot Chat session - live status, PR tracking, and one-click jump to the exact chat. Pure display overlay (no PTY, server, or queueing); rows carry `row_type == 'cursor_agent_gui'` / `'vscode_copilot_gui'`. VS Code "close" is an honest Leap-side row removal (UUID-keyed `vscode_gui_hidden` pref, auto-returns on new activity) - VS Code exposes no reliable archive/close-by-id to an extension; the scan also drops chats the user archived in VS Code itself (read from `agentSessions.state.cache`). Full design - the on-disk scans, status mappings, jump via the shared Leap extension, synthetic-row reconciliation, and the two close buttons - is in the `cursor-editor-agent-tabs` skill.
