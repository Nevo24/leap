"""
Socket client for Leap.

Handles communication with the Leap server via Unix socket.
"""
...
class SocketClient:
    """Client for communicating with Leap server via Unix socket."""
    ...
    def _send_request(
        ...
        try:
            ...
            if not silent and self._should_print_error():
                print(f"Error: Cannot connect to Leap server")
                print(f"  → Check if there's a terminal running: lps {self.tag}")
                print(f"  → Details: {e}")
            return None
            ...
            if not silent and self._should_print_error():
                ...
                print(
                    f"  → The Leap server (tag: {self.tag}) may be in a bad state"
                )
                print(
