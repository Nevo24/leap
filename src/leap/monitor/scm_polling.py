...
class SessionRefreshWorker(QThread):
    ...
    ``vscode_hidden`` (rows the user removed from Leap, sessionId ->
    dismiss-time) and ``vscode_keep_ids`` (PR-tracked VS Code session ids)
    are GUI-thread snapshots passed in at construction so the worker
    thread never touches the monitor's mutable state.
    """
