...
**A queueing system and dashboard for managing multiple AI CLI sessions + in-editor agents (Cursor Agent tabs, VS Code Copilot Chat)**
...
- **Context usage tracking** - See how full each session's context window is (all five CLIs - Claude, Codex, Copilot, Cursor Agent, Gemini - plus VS Code Copilot Chat rows), so you know how close it is to auto-compaction. Hover the Context cell on a Claude, Codex, or Gemini session to also see last-message and whole-session token counts plus an estimated API cost
